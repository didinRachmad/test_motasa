export default {
            ...
            columns: [
                ...
                { data: "name", name: "name" },
                {
                    ...
                    render: function (data, type, row) {
                        let buttons = "";
                        if (row.can_edit_permission) {
                            buttons += `<a href="${row.edit_permission_url}" class="btn btn-sm btn-info rounded-4" data-bs-toggle="tooltip" data-bs-title="Atur Permissions"><i class="bi bi-sliders"></i></a> `;
                        }
                        if (row.can_edit) {
    ...
    initShow() {
        console.log("Halaman Roles Show berhasil dimuat!");
    },
    initCreate() {
        console.log("Halaman Roles Create berhasil dimuat!");
    },
    initEdit() {
        console.log("Halaman Roles Edit berhasil dimuat!");
        window.toggleMenuPermissionsCheckbox = function (
            masterCheckbox,
            menuId
        ) {
            var checkboxes = document.querySelectorAll(
                ".menu-permission-checkbox-" + menuId
            );
